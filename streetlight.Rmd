...
mf_com_melt <- melt(mf_com,measure.vars = colnames(mf_com)[2:15],id.vars = colnames(mf_com)[c(1,16:22)])

mf_com_melt$group <- ifelse(grepl("Origin",mf_com_melt$variable),"From",
                            ifelse(grepl("Middle",mf_com_melt$variable),"Through",
                            ifelse(grepl("Destination",mf_com_melt$variable),"To",NA)))
```
...
```{r}
trueCentroids <- as.data.frame(gCentroid(shape_org,byid = TRUE))
```
